...
| 6 — Fiscal e Tributário | Configuração tributária, documentos fiscais, fila de emissão e contingência | — |
...
- **Banco**: 29 migrations aplicam limpas do zero, com **379 asserções** cobrindo isolamento por RLS entre estabelecimentos e entre clientes, regras de negócio (preço recalculado no servidor, FIFO/FEFO, CMV histórico, conciliação de caixa, numeração fiscal sem buraco) e integridade (constraints, triggers de derivação de `tenant_id`).
- **API**: **233 testes** com `fastify.inject`, cobrindo contratos de entrada e saída, autenticação, autorização por permissão, e o mapeamento de erros de negócio para status HTTP.
